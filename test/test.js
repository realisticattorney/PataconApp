...
contract("dBank", ([deployer, user]) => {
  ...
  describe("testing token contract...", () => {
    describe("success", () => {
      it("checking token name", async () => {
        expect(await token.name()).to.be.eq("Patacon");
      });

      it("checking token symbol", async () => {
        expect(await token.symbol()).to.be.eq("MNM");
      });
  ...
  describe("testing deposit...", () => {
    ...
    describe("success", () => {
      beforeEach(async () => { //it will deposit from the 2nd user on the account lists ganache provides
